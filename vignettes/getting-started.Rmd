---
title: "Getting started with clusteredMS"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Getting started with clusteredMS}
...
`clusteredMS` provides nonparametric analysis of clustered multistate
process data, based on Bakoyannis (2021). It exposes one user-facing
function, `patp()`, that:
...
The package depends only on `survival`. It is self-contained and does
not require the `mstate` package.

## Input data format

`clusteredMS` expects data in **interval format**: one row per
...
contiguous (`Tstop[k] == Tstart[k+1]`) and spatially contiguous
(`Sstop[k] == Sstart[k+1]`).
...
`fit$curves` contains the time-indexed point estimate, standard error,
pointwise confidence interval, and (with `cband = TRUE`) simultaneous
band limits.
...
When `s > 0` and the Markov assumption is implausible, use
`LMAJ = TRUE`:
...
When cluster size is potentially informative (e.g., sicker centers
contribute more patients), use the weighted estimator:
...
work, then switch to `B = 1000` for publication-quality inference.
