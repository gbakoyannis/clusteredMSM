...
#'   \code{fn}.
#' @param cluster Character. Name of the cluster ID column.
...
#'
#' @export
cluster_boot <- function(data, cluster, B, fn, ...,
                         strata = NULL, seed = NULL, verbose = FALSE) {

  if (!is.data.frame(data)) stop("'data' must be a data frame")
  if (!cluster %in% names(data)) stop("column '", cluster, "' not found in data")
  ...
  if (is.null(strata)) {
    ...
    missing_in_strata <- setdiff(cluster_chr, names(strata))
    ...
    strata_aligned <- unname(strata[cluster_chr])
  ...
  cluster_rows <- split(seq_len(nrow(data)), data[[cluster]])
